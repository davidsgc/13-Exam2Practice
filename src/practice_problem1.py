...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if len(contents) > volume:
            ...
            self.volume = volume
        self.og_contents = self.contents
        self.og_volume = self.volume
        self.history = []
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > new_volume:
            for k in range(new_volume, len(self.contents)):
                outside = outside + self.contents[k]
            for k in range(self.volume):
                inside = inside + self.contents[k]
            self.contents = inside
        return outside
    ...
    def double_then_shrink(self, new_volume):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # ---------------------------------------------------------------------
        doubled = len(self.double())
        return (doubled + len(self.shrink(new_volume)))
